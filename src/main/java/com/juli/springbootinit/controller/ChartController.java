...
public class ChartController {
    ...
    public BaseResponse<BiResponse> genChartByAi(@RequestPart("file") MultipartFile multipartFile,
        ...
        //AI模型id
        long modelId = 1721129941246390274L;
        // 获取字段
        ...
        // 拆分分析结果
        String[] splits = result.split("分析结论：");
        // 校验拆分结果
        if (splits.length < 2) {
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "AI 生成错误");
        }
        String genResult = splits[1].trim(); // 分析结果
        String[] chartJson = splits[0].trim().split("前端图表代码：");
        String genChart = chartJson[1].trim(); // 图表代码
        // 保存到数据库
...
//        chartService.validChart(chart, false);
    ...
    private QueryWrapper<Chart> getQueryWrapper(ChartQueryRequest chartQueryRequest) {
        ...
        // 拼接查询条件
        queryWrapper.eq(id != null && id > 0, "id", id);//注意判空
        queryWrapper.eq(StringUtils.isNotBlank(goal), "goal", goal);
        queryWrapper.like(StringUtils.isNotBlank(name), "name", name);//注意判空
        if (userId == 0) { // 管理员查询所有数据不需要设置userID
        }
        else { // 普通用户查询个人数据
